...
export default function QualityPipeline() {
  ...
  const frontierModels = summary?.frontierModels || [];
  const frontierModelOpportunityList = summary?.frontierModelOpportunities || [];
  ...
  const frontierModelLabels = donutLabels(frontierModels);
  const offeringStyle = donutStyle(offerings);
  const campaignStyle = donutStyle(campaigns);
  const frontierModelStyle = donutStyle(frontierModels);

  return <><header><svg width="24" height="24" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24" aria-hidden="true"><path d="M4 19V5m0 14h16M8 17V9m4 8V7m4 10v-5m4 5V4" /></svg><h1>Quality Pipeline</h1></header><main className="container quality-pipeline-page">{isLoadingSummary ? <LoadingSpinner label="Loading BCM and Insurance pipeline…" /> : <><h2>Quality of Pipeline</h2><table className="quality-summary"><thead><tr><th /><th>Q3</th><th>Q4</th><th>2026</th><th>2026+</th><th>Total</th></tr></thead><tbody>{['Qualified', 'Unqualified'].map((label) => { const values = summary?.rows.find((row) => row.label === label); return <tr key={label}><th>{label}</th>{periods.map((period) => <td key={period}>{values ? formatMoney(values[period]) : '—'}</td>)}</tr>; })}</tbody></table>{summaryError && <p className="quality-summary-error">{summaryError}</p>}<section className="quality-section"><h2>Pipeline Composition</h2><div className="quality-composition"><Donut title="By Offerings" labels={offeringLabels} style={offeringStyle} /><Donut title="By Frontier Model" labels={frontierModelLabels.length ? frontierModelLabels : ['No data loaded']} style={frontierModelStyle} /><Donut title="By Campaign" labels={campaignLabels} style={campaignStyle} /></div></section><section className="quality-section quality-opportunities"><h2>Top 6 Opportunities</h2><div className="quality-opportunity-grid"><div className="quality-opportunity-panel"><h3>Across Top Offerings</h3><OpportunityTable opportunities={opportunityList} /></div><div className="quality-opportunity-panel"><h3>Across Frontier Models</h3><OpportunityTable opportunities={frontierModelOpportunityList} /></div><div className="quality-opportunity-panel"><h3>Across Top Campaign Themes</h3><OpportunityTable opportunities={campaignOpportunityList} /></div></div></section></>}</main></>;
}
