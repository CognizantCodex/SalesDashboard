...
export default function SalesPipelineDetails({ pipelineResult, onBack, entityLabel = 'SLS' }) {
  ...
  const dealTypeOptions = useMemo(() => {
    const dealTypes = new Set((pipelineResult?.rows || []).map((row) => row.dealType || 'Unspecified'));
    return ['All', ...Array.from(dealTypes).sort()];
  }, [pipelineResult]);
  const filteredAccounts = useMemo(() => buildFilteredAccounts(pipelineResult?.rows || [], dealTypeFilter), [pipelineResult, dealTypeFilter]);
  const filteredPipelineResult = useMemo(
    () => buildFilteredPipelineResult(pipelineResult, filteredAccounts, dealTypeFilter),
    [pipelineResult, filteredAccounts, dealTypeFilter]
  );
  const accountRows = useMemo(() => {
    if (!filteredAccounts?.length) return [];
    return filteredAccounts.flatMap((account) => {
  ...
  function exportExcel() {
    if (!filteredAccounts?.length) {
      setError('Pipeline details are not available to export.');
      return;
    }
  ...
  function buildPipelineSheet(accounts) {
    const sheetRows = [{
      ...
      cells: [
        ...
        { column: 3, value: 'Total Pipeline', style: 1 },
        { column: 4, value: 'Qualified Pipeline', style: 1 },
        { column: 5, value: 'Un-Qualified Pipeline', style: 1 }
      ]
    }];
    const merges = [];
    let rowIndex = 2;
    ...
    accounts.forEach((account) => {
      ...
      grandTotal.pipeline += account.pipeline || 0;
      grandTotal.qualified += account.qualified || 0;
      grandTotal.unqualified += account.unqualified || 0;
      ...
      practices.forEach((practice, practiceIndex) => {
        sheetRows.push({
          ...
          cells: [
            ...
            { column: 3, value: practice.labels.pipeline, style: 4 },
            { column: 4, value: practice.labels.qualified, style: 4 },
            { column: 5, value: practice.labels.unqualified, style: 4 }
          ]
        });
        rowIndex += 1;
      });
      ...
      sheetRows.push({
        ...
        cells: [
          ...
          { column: 3, value: account.labels.pipeline, style: 5 },
          { column: 4, value: account.labels.qualified, style: 5 },
          { column: 5, value: account.labels.unqualified, style: 5 }
        ]
      });
    ...
    sheetRows.push({
      ...
      cells: [
        ...
        { column: 3, value: formatPipelineMoney(grandTotal.pipeline), style: 5 },
        { column: 4, value: formatPipelineMoney(grandTotal.qualified), style: 5 },
        { column: 5, value: formatPipelineMoney(grandTotal.unqualified), style: 5 }
      ]
    });
  ...
  return (
    <>
      <header>
        ...
        <h1>Pipeline Details <span>CY {pipelineResult?.year || new Date().getFullYear()}</span></h1>
      </header>

      <main className="container detail-container">
        <button className="back-link" onClick={onBack}>Back to Dashboard</button>

        {pipelineResult?.matchedSlsNames?.length > 0 && (
          <div className="chips detail-chips">
            {pipelineResult.matchedSlsNames.map((name) => <span className="chip" key={name}>{name}</span>)}
          </div>
        )}

        <SalesSummary pipelineSummary={filteredPipelineResult} />

        <div className="detail-filter-row">
          <label htmlFor="grouped-deal-type">Grouped Deal Type</label>
          <select id="grouped-deal-type" value={dealTypeFilter} onChange={(event) => setDealTypeFilter(event.target.value)}>
        ...
        {!filteredAccounts?.length ? (
          ...
          <section className="table-wrap detail-table-wrap">
            <div className="table-header">
              <h2>{pipelineResult.query} - Account & Practice Breakdown</h2>
                ...
                {accountRows.map((row, index) => (
                  row.type === 'account-heading' ? (
                    ...
                    <tr key={row.type + '-' + row.account + '-' + (row.practice || index)} className={row.type === 'account-total' ? 'total-row' : 'detail-row'}>
                      ...
                      <td>{row.labels.pipeline}</td>
                      <td>{row.labels.qualified}</td>
                      <td>{row.labels.unqualified}</td>
                    </tr>
                  )
                ))}
              </tbody>
...
function buildFilteredAccounts(rows, dealTypeFilter) {
  const details = new Map();
  rows.forEach((row) => {
    const dealType = row.dealType || 'Unspecified';
    if (dealTypeFilter !== 'All' && dealType !== dealTypeFilter) return;
    ...
    detail.pipeline += row.pipeline || 0;
    detail.qualified += row.qualified || 0;
    detail.unqualified += row.unqualified || 0;
  ...
  Array.from(details.values())
    ...
    .forEach((detail) => {
      const labeledDetail = {
        ...detail,
        labels: {
          pipeline: formatPipelineMoney(detail.pipeline),
          qualified: formatPipelineMoney(detail.qualified),
          unqualified: formatPipelineMoney(detail.unqualified)
        }
      };
  ...
  return Array.from(accountMap.values()).map((account) => ({
    ...account,
    labels: {
      pipeline: formatPipelineMoney(account.pipeline),
      qualified: formatPipelineMoney(account.qualified),
      unqualified: formatPipelineMoney(account.unqualified)
    }
  }));
}

function buildFilteredPipelineResult(pipelineResult, accounts, dealTypeFilter) {
  if (!pipelineResult) return null;
  const totals = accounts.reduce(
    (sum, account) => ({
      pipeline: sum.pipeline + account.pipeline,
      qualified: sum.qualified + account.qualified,
      unqualified: sum.unqualified + account.unqualified,
      rows: sum.rows + account.rows
    }),
    { pipeline: 0, qualified: 0, unqualified: 0, rows: 0 }
  );
  return {
    ...pipelineResult,
    dealTypeFilter,
    ...
    metrics: {
      ...
      labels: {
        pipeline: formatPipelineMoney(totals.pipeline),
        qualified: formatPipelineMoney(totals.qualified),
        unqualified: formatPipelineMoney(totals.unqualified)
      }
    }
  };
}
...
function workbookXml() {
  return '<?xml version="1.0" encoding="UTF-8" standalone="yes"?><workbook xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships"><sheets><sheet name="Pipeline" sheetId="1" r:id="rId1"/></sheets></workbook>';
}
...
function coreXml() {
  return '<?xml version="1.0" encoding="UTF-8" standalone="yes"?><cp:coreProperties xmlns:cp="http://schemas.openxmlformats.org/package/2006/metadata/core-properties" xmlns:dc="http://purl.org/dc/elements/1.1/" xmlns:dcterms="http://purl.org/dc/terms/" xmlns:dcmitype="http://purl.org/dc/dcmitype/" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"><dc:title>Pipeline Export</dc:title><dc:creator>SLS Dashboard</dc:creator></cp:coreProperties>';
}
