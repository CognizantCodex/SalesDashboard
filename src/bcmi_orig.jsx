...
function OpportunityPanel({ period, opportunities, gap }) {
  const rows = opportunities?.rows || [];
  return <section className="bcmi-orig-opportunity"><h2>{period}</h2><dl><div><dt>GAP</dt><dd>{gap != null ? formatRaAchieved(gap) : '—'}</dd></div><div><dt>Total Opportunity Size</dt><dd>{formatRa(opportunities?.totalTcv)}</dd></div></dl><p>Top 10 Opportunities by TCV</p><table><thead><tr><th>#</th><th>Act Name</th><th>Description</th><th>Total TCV</th></tr></thead><tbody>{ranks.map((rank, index) => { const row = rows[index]; return <tr key={rank}><td>{rank}</td><td>{row?.account || ''}</td><td>{row?.description || ''}</td><td>{row ? formatRa(row.totalTcv) : ''}</td></tr>; })}</tbody></table></section>;
}
...
function formatRaAchieved(value) {
  return `$${(Number(value || 0) / 1_000_000).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}M`;
}

function formatForecastRa(value) {
  return `$${(Number(value || 0) / 1000).toLocaleString(undefined, { minimumFractionDigits: 1, maximumFractionDigits: 1 })}M`;
}
...
export default function BcmiOrig() {
  ...
  const achievedByPeriod = raAchieved && {
    aug: 0,
    q3: Number(raAchieved.q3 || 0),
    q4: Number(raAchieved.q4 || 0),
    year: Number(raAchieved.q3 || 0) + Number(raAchieved.q4 || 0)
  };
  const gapByPeriod = achievedByPeriod && raSummary && Object.fromEntries(
    Object.keys(achievedByPeriod).map((period) => [period, Number(raSummary[period] || 0) * 1000 - achievedByPeriod[period]])
  );
